...
const listPointer = path => typeof path === 'string' && /^\/[A-Za-z][A-Za-z0-9_-]*$/.test(path)
const pointer = (value, path) => listPointer(path)
  && object(value) && Object.hasOwn(value, path.slice(1)) ? value[path.slice(1)] : undefined

function validateDataSource(source) {
  if (source === undefined || source === null) return
  if (source.descriptor?.method !== 'GET' || !listPointer(source.descriptor.rowsPointer)
    || !listPointer(source.descriptor.rowIdentityPointer)) throw Error('pack_data_source_unsupported')
}
...
export function createPackActionRuntime(transport = { send }, uuid = () => crypto.randomUUID()) {
  ...
  function scopedTransport(scope) {
    return { async send(path, method, body, contentType, headers) {
      assertCurrent(scope)
      const response = await transport.send(path, method, body, contentType, headers, { signal: scope.controller.signal })
      // Transport implementations may finish despite cancellation. Never publish their stale result.
      assertCurrent(scope)
      return response
    } }
  }
  ...
  async function readEntry(scope, kind, id, version) {
    if (!identifier(id) || (version !== undefined && !identifier(version))) throw Error('pack_definition_invalid')
    const response = await scopedTransport(scope).send(`/api/local-node/catalogue/definitions/${kind}/${encodeURIComponent(id)}${version ? `?version=${encodeURIComponent(version)}` : ''}`)
    assertCurrent(scope)
  ...
  async function refresh(scope) {
    ...
    const response = await dispatchPackRequest({ dispatch: { schemaVersion: 1, kind: 'request', ...source } }, {}, scopedTransport(scope))
    assertCurrent(scope)
  ...
  return {
    ...
    async load(viewId) {
      ...
      try {
        const entry = await readEntry(scope, 'ViewDefinition', viewId)
        ...
        validateDataSource(entry.renderPlan.bindings.dataSource)
    ...
    async begin(id) {
      ...
      try {
        ...
        if (action.inputForm) {
          const entry = await readEntry(scope, 'FormDefinition', action.inputForm.formId, action.inputForm.version)
          assertCurrent(scope)
      ...
      } catch (error) { if (isCurrent(scope)) { state.error = error.message; state.activeAction = null } }
      finally { if (isCurrent(scope)) state.busy = false }
      return snapshot()
    },
    ...
    async invoke(values = {}, file) {
      ...
      try {
        ...
        const response = await dispatchPackRequest(state.activeAction, sources, scopedTransport(scope))
        assertCurrent(scope)
        state.receipt = normalizePackReceipt(response)
        if (state.receipt.status >= 200 && state.receipt.status < 300 && state.activeAction.result?.refresh === 'data-source') {
          const receipt = state.receipt
          try { await refresh(scope) } finally { if (isCurrent(scope)) state.receipt = receipt }
          ...
          const reload = this.load(state.plan.definitionId)
          const refreshedScope = currentScope
          await reload
          assertCurrent(refreshedScope)
          state.receipt = receipt; state.navigationRevision = revision + 1
        }
      } catch (error) { if (isCurrent(scope)) state.error = error.message }
      finally { if (isCurrent(scope)) state.busy = false }
      return snapshot()
    },
