...
/// <summary>
/// Verifies the data exchange administration page's list and read-only detail interactions.
/// </summary>
public sealed class DataExchangeAdminPageTests : BunitContext
{
    [Fact]
    public void List_renders_the_fixture_definition_rows()
    {
        var cut = RenderPage(new FixtureDataExchangeAdminClient());

        cut.WaitForAssertion(() =>
        {
            Assert.Equal(3, cut.FindAll("div.hl-data-grid__row--leaf").Count);
            var trialBalance = cut.Find("div.hl-data-grid__row--leaf[data-row-id='bank-feed-import@2.1.0']");
            Assert.Equal("bank-feed-import", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='key']")!.TextContent.Trim());
            Assert.Equal("Bank feed import", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='title']")!.TextContent.Trim());
            Assert.Equal("2.1.0", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='version']")!.TextContent.Trim());
            Assert.Equal("import", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='kind']")!.TextContent.Trim());
            Assert.Equal("Tenant", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='cascadeLayer']")!.TextContent.Trim());
            var legacyLedgerExport = cut.Find("div.hl-data-grid__row--leaf[data-row-id='legacy-ledger-export@0.3.0']");
            Assert.Equal("export", legacyLedgerExport.QuerySelector("div[role='gridcell'][data-column-id='kind']")!.TextContent.Trim());
        });
    }

    [Fact]
    public void Detail_renders_the_Data_Exchange_h1_with_h2_section_headings()
    {
        var cut = RenderPage(new FixtureDataExchangeAdminClient());
        ClickDefinition(cut, "bank-feed-import");

        cut.WaitForAssertion(() =>
        {
            var panel = cut.Find("aside[data-detail-panel]");
            Assert.Equal("Data Exchange", cut.Find("h1").TextContent.Trim());
            Assert.Equal(
                ["Settings", "Provenance"],
                panel.QuerySelectorAll("h2").Select(heading => heading.TextContent.Trim()));
            Assert.Equal("Data exchange definition bank-feed-import", panel.GetAttribute("aria-label"));
            AssertDefinitionValue(panel, "Key", "bank-feed-import");
            AssertDefinitionValue(panel, "Version", "2.1.0");
            AssertDefinitionValue(panel, "Kind", "import");
            AssertDefinitionValue(panel, "Cascade layer", "Tenant");
            AssertDefinitionValue(panel, "Schema version", "1");
            Assert.Contains("csv-standard", panel.QuerySelector("pre[data-testid='settings-json']")!.TextContent);
            Assert.Contains("harborline.core-exchange", panel.QuerySelector("pre[data-testid='provenance-json']")!.TextContent);
    ...
    {
        var cut = RenderPage(new FixtureDataExchangeAdminClient());
        ClickDefinition(cut, "legacy-ledger-export");
    ...
    private IRenderedComponent<DataExchangeAdminPage> RenderPage(IDataExchangeAdminClient client)
    {
        ...
        return Render<DataExchangeAdminPage>(settings =>
            settings.Add(page => page.PanelRailCapable, true));
    }

    private static void ClickDefinition(IRenderedComponent<DataExchangeAdminPage> cut, string key)
    {
