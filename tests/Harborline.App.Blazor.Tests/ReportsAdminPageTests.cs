...
/// <summary>
/// Verifies the reports administration page's list and read-only detail interactions.
/// </summary>
public sealed class ReportsAdminPageTests : BunitContext
{
    [Fact]
    public void List_renders_the_fixture_definition_rows()
    {
        var cut = RenderPage(new FixtureReportsAdminClient());

        cut.WaitForAssertion(() =>
        {
            Assert.Equal(3, cut.FindAll("div.hl-data-grid__row--leaf").Count);
            var trialBalance = cut.Find("div.hl-data-grid__row--leaf[data-row-id='trial-balance@2.1.0']");
            Assert.Equal("trial-balance", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='key']")!.TextContent.Trim());
            Assert.Equal("Trial balance", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='title']")!.TextContent.Trim());
            Assert.Equal("2.1.0", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='version']")!.TextContent.Trim());
            Assert.Equal("standard", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='kind']")!.TextContent.Trim());
            Assert.Equal("Tenant", trialBalance.QuerySelector("div[role='gridcell'][data-column-id='cascadeLayer']")!.TextContent.Trim());
            var occupancy = cut.Find("div.hl-data-grid__row--leaf[data-row-id='occupancy@0.3.0']");
            Assert.Equal("snapshot", occupancy.QuerySelector("div[role='gridcell'][data-column-id='kind']")!.TextContent.Trim());
        });
    }

    [Fact]
    public void Detail_renders_the_Reports_h1_with_h2_section_headings()
    {
        var cut = RenderPage(new FixtureReportsAdminClient());
        ClickDefinition(cut, "trial-balance");

        cut.WaitForAssertion(() =>
        {
            var panel = cut.Find("aside[data-detail-panel]");
            Assert.Equal("Reports", cut.Find("h1").TextContent.Trim());
            Assert.Equal(
                ["Parameters", "Provenance"],
                panel.QuerySelectorAll("h2").Select(heading => heading.TextContent.Trim()));
            Assert.Equal("Report definition trial-balance", panel.GetAttribute("aria-label"));
            AssertDefinitionValue(panel, "Key", "trial-balance");
            AssertDefinitionValue(panel, "Version", "2.1.0");
            AssertDefinitionValue(panel, "Kind", "standard");
            AssertDefinitionValue(panel, "Cascade layer", "Tenant");
            AssertDefinitionValue(panel, "Schema version", "1");
            Assert.Contains("chart-7", panel.QuerySelector("pre[data-testid='parameters-json']")!.TextContent);
            Assert.Contains("harborline.core-reports", panel.QuerySelector("pre[data-testid='provenance-json']")!.TextContent);
    ...
    private IRenderedComponent<ReportsAdminPage> RenderPage(IReportsAdminClient client)
    {
        ...
        return Render<ReportsAdminPage>(parameters =>
            parameters.Add(page => page.PanelRailCapable, true));
    }

    private static void ClickDefinition(IRenderedComponent<ReportsAdminPage> cut, string key)
    {
